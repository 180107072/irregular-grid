class_name StandaloneOperationsAdd


static func add_quad_face(top: Topology, a: Vector3, b: Vector3, c: Vector3, d: Vector3):
...
static func add_quad_subdivide(top, a: Vector3, b: Vector3, c: Vector3, d: Vector3):
	var ab = lerp(a, b, 0.5)
	var bc = lerp(b, c, 0.5)
	var cd = lerp(c, d, 0.5)
	var da = lerp(d, a, 0.5)
...
static func add_subdivide_tri(top, a, b, c, div = 2):
...
static func add_tri_subdivide_face(top, a, b, c):
	var ab = lerp(a, b, 0.5)
	var bc = lerp(b, c, 0.5)
	var ca = lerp(c, a, 0.5)
	var cp = (a + b + c) / 3
